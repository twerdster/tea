...
%Speed:
%T11: Need to test histogram sizes: 7,32,64,128,256  (6 to 10 sizes, 2 machines, 1 GPU) = (20 Tests)
baseTestDir='20-100-';
numClasses=[2,7,32,64,65,256,257,1000];
machines= { ...
...
T.numS=100*1e6;
T.numT=10;
T.comment='Test: Timing different number of classes';
% --------------------------------
for i = 2:2:length(machines)
    ...
    for c = numClasses
        ...
        % --------------------------------
        baseDir = [machines{i} baseTestDir num2str(c) '-char']  ;
        T.baseDir=baseDir;
        fprintf('%s\n',genTestString(T));
        fprintf(f,'%s\n',genTestString(T));
        % --------------------------------
