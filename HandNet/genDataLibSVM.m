...
function genDataLibSVM(dataDir, outDir, outName, nFeatures, probeSize, featurePrecision, rseed)
...
for imageIndex = 1:nImages
    ...
    for r=1:size(data,1)
        fprintf(fData,'%i ',labels(r));
        fprintf(fData,' %i:%f', [data(r,:) ; classInds ]);
        fprintf(fData,'\n');
    end
    if mod(imCnt,100) == 0
        toc
